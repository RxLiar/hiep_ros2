"""
main.py — AGV Busan Autonomous Robot HMI v4.3.2

Fixes:
- Bắt đầu Login sau khi QApplication event loop đã chạy.
- Ctrl+C và SIGTERM thoát ổn định.
- Đổi người vận hành bằng queued call, tránh nested re-entry trong signal.
- Session shutdown idempotent, ROS chỉ shutdown một lần.
"""
import signal
import sys
...
class Session:
    ...
    def _on_switch_user(self):
        """Đóng window cũ rồi mở Login ở event-loop turn kế tiếp."""
        ...
        # Đợi signal handler kết thúc và Qt xử lý deleteLater().
    ...
    def shutdown(self):
        """Dừng UI/process và ROS đúng một lần."""
        if self._shutdown_done:
            return
        ...
        if window is not None:
            try:
                ...
                print(f"[HMI] Window shutdown failed: {e}")

        if ros is not None:
            try:
                ros.shutdown()
            except Exception as e:
                print(f"[HMI] ROS shutdown failed: {e}")
...
def main():
    ...
    # Qt chạy phần lớn trong C++; timer giúp Python xử lý Unix signals đều.
    ...
    # Quan trọng: quit() từ Login reject chỉ đáng tin khi app.exec() đã chạy.
