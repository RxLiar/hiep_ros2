"""
ros_node.py — AGV Busan Autonomous Robot HMI

Thêm:
  - nav2_status_signal: subscribe /navigate_to_pose/_action/status
    và /bt_navigator/transition_event để hiển thị trạng thái
    planner/controller lên ErrorHeader khi navigation/routes.

FIX (v4.2.7):
  - _GOAL_STATUS[2] (EXECUTING) trước đây map sang level "ok", nhưng
    MainWindow.on_nav2_status_update() lại coi level "ok" là "xoá banner"
    (clear_nav2()). Kết quả: message "Nav2: Đang thực thi..." KHÔNG BAO
    GIỜ hiển thị được trên ErrorHeader — bug im lặng.
    Sửa: EXECUTING -> None (không cần banner khi đang chạy bình thường,
    chỉ cảnh báo/lỗi mới cần chiếm chỗ trên header).
"""
...
class RosInterface(QObject):
    ...
    def __init__(self, args=None):
        ...
        self.node.get_logger().info("AGV HMI node started (spin thread active)")

        self.node.create_subscription(
            Path,
            "/plan",          # Nav2 publish planned path tại đây
            self._path_cb,
            10,
        )
