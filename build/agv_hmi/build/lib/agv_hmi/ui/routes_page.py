"""
routes_page.py — Routes screen v2.4

[NEW so với v2.3]
  - Repeat / Lặp lại lộ trình: chạy 1 lần / N lần / vô hạn.
    + Mỗi chu kỳ chạy xong (success) sẽ tự động nghỉ _repeat_pause_ms
      rồi chạy lại nếu còn lượt (đếm) hoặc chưa Stop (vô hạn).
    + Mỗi chu kỳ được ghi thành MỘT record riêng trong mission_logger
      (dùng lại start_record/finish_record sẵn có) — tab Lịch sử và
      thống kê "Run X / Cargo Y" tự cộng dồn không cần đổi mission_logger.
    + Nếu mission thất bại (failed) giữa chừng, chuỗi lặp DỪNG LUÔN
      (không lặp lại lỗi vô ích) — operator phải bấm Start lại.
  - Chặn đổi route khi đang chạy/lặp (tránh log lẫn lộn giữa 2 route).
...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QSpinBox,   # [NEW] cho repeat combo/spin
)
