"""
main_window.py — AGV Busan Autonomous Robot v4.2.7
...
class Sidebar(QWidget):
    ...
    def _rebuild_nav(self):
        ...
        engineer_only = {self.IDX_MAPPING, self.IDX_MAPLIB, self.IDX_SETTINGS}
        pages = [
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        self._settings = SettingsPage(theme_mgr=self._theme_mgr)

        for p in (self._home, self._mapping, self._nav, self._routes,
                  self._conveyor, self._maplib, self._settings):
    ...
    def _switch_page(self, idx: int):
        ...
        if old_idx is not None and old_idx != idx:
            self._cleanup_leaving_page(old_idx)
